...
4. 可靠性
5. 安全性
6. 支持多线程（C++11 开始（2011 年的时候）,C++就引入了多线程库，在 windows、linux、macos 都可以使用`std::thread`和`std::async`来创建线程）
...
- 重写发生在`运行期`，是`子类`对`父类`的允许访问的方法的实现过程进行`重新编写`
